...
edb <- hub[["AH60977"]]  ## downloaded once
# organism(edb)
...
## Peptide table
peptab <- fread('~/Downloads/CLL_project/data/Methylation_search/peptides_table.txt')
...
## Methylation pattern
methyl_pattern <- '-215.147'
idx_methyl <-  grep(pattern = methyl_pattern, peptab$`Peptide sequence`)


# Reorder
samples_annot <- samples_annot[match(colnames(non_replicates), samples_annot$Labels_proteomics), ]
non_replicates_methyl <- non_replicates[idx_methyl, ]
...
tsne.pca <- function(dat) {
  ...
  pdf(paste0('~/Downloads/CLL_project/code/figures/CLL_proteomics_PCA_methylation.pdf'), width = 6, height = 5)
  plot(p)
  dev.off()
  ...
  p <- ggplot(data = tsne_dat, aes(x = tsne.1, y = tsne.2,  col = as.factor(cluster))) + 
    ...
    # geom_label_repel(aes(label = samples), size = 2) + 
    scale_color_manual(name = '', values = col_list) +
    labs(x = 'TSNE-1', y = 'TSNE-2',  title = 'Methylation') + 
    guides(col = guide_legend(override.aes = list(fill = col_list, size = 2))) +
    theme_bw() + 
    theme(plot.title = element_text(hjust = 0.5))
  ...
  pdf(paste0('~/Downloads/CLL_project/code/figures/CLL_proteomics_TSNE_methylation.pdf'), width = 6, height = 5)
  plot(p)
  dev.off()
...
# tsne.pca(dat = non_replicates_methyl)
...
non_replicates_methyl <- non_replicates_methyl[apply(non_replicates_methyl, 1 , function(i) sum(!is.na(i)) > 18), ]
...
## Normalize by relative protein abundance
# # Merge by protein level
# dat_summarized <- lapply(list(non_replicates_methyl, non_replicates), function(dat) {
#   
...
# })
# 
# non_replicates_methyl_summarized <- dat_summarized[[1]]
# non_replicates_summarized <- dat_summarized[[2]]
# 
# common_genes <- intersect(rownames(non_replicates_methyl_summarized), rownames(non_replicates_summarized))
# 
# non_replicates_methyl_summarized <- non_replicates_methyl_summarized[common_genes, ]
# non_replicates_summarized <- non_replicates_summarized[common_genes, ]
# 
# 
# # Divide methylated by total relative quantification
# normalized_summarized <- non_replicates_methyl_summarized - non_replicates_summarized
# 

tofit <- non_replicates_methyl
...
p1 <- ggplot(subset(toplot, L1 == paste0('FDR - ', levToChoose)), aes(x = Cluster, y = value, fill = Cluster)) + 
  ...
  # facet_grid(.~L1) +
  labs(x = '', y = 'Differentially abundant peptides \n(Adjusted p-value < 0.1)', title = 'Methylation')+ 
  scale_y_continuous(expand = c(0,0, 0.1, 0), breaks = seq(0, max(toplot$value), 10)) + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5), 
...
# save(p, file = '~/Downloads/CLL_project/code/figures/CLL_methylated_pvalue.Rdata')

pdf(paste0('~/Downloads/CLL_project/code/figures/CLL_methylated_pvalue.pdf'), width = 4, height = 4)
plot(p1)
dev.off()
...
p <- ggplot(aggregated_plots, aes(x = Cluster, y = value, fill = Dir)) +
  ...
  facet_grid(.~L1) +
  labs(x = '', y = 'Number of modified differentially abundant peptides', title = 'Methylation') +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),  plot.title = element_text(hjust = 0.5))

pdf('~/Downloads/CLL_project/code/figures/CLL_methylated_FCDirection.pdf', width = 6, height = 5)
plot(p)
dev.off()
...
p2 <- ggplot(subset(resLimma, Cluster == 'PG5'), aes(x = logFC, y = -log10(adj.P.Val))) +
  ...
  labs(x = 'Log2 fold change', y = 'Adjusted p-value (-log10)', title = 'Methylation') + 
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5),
        panel.grid = element_blank(),
        strip.background = element_rect(fill = "white"), 
...
pdf('~/Downloads/CLL_project/code/figures/CLL_methylated_Volcano_PG5.pdf', width = 4, height = 5)
plot(p2)
dev.off()


pdf('~/Downloads/CLL_project/code/figures/CLL_methylated_combined.pdf', width = 5, height = 7)
plot.with.inset <-
  ggdraw() +
  draw_plot(p2) +
  draw_plot(p1, x = 0.5, y = 0.5, width = 0.4, height = 0.4)
plot(plot.with.inset)
dev.off()

save(plot.with.inset, file = '~/Downloads/CLL_project/code/figures/CLL_methylated_combined.RData')
...
pdf('~/Downloads/CLL_project/code/figures/CLL_methylated_Volcano.pdf', width = 15, height = 4)
plot(p)
dev.off()
...
pdf('~/Downloads/CLL_project/code/figures/CLL_methylated_diff_abund_sig.pdf', width = 20, height = 20)
...
write.table(resLimma, '~/Downloads/CLL_project/data/CLL_methylated_diff_abund.txt', sep = '\t', row.names = FALSE)
# write.table(resLimma[resLimma$adj.P.Val < thres, ], '~/Downloads/CLL_project/data/CLL_methylated_diff_abund_Sig.txt', sep = '\t', row.names = FALSE)
