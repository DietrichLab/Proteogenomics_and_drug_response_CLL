...
tmp_BCR <- wideFormat(multiomics_MAE[BCR_genes, ,"proteomics"], colDataCols = c("PG", "IGHV" )) %>% 
  as_tibble()
tmp_BCR_mx <- tmp_BCR %>% dplyr::select(-primary, -PG, -IGHV) %>% as.matrix()
rownames(tmp_BCR_mx) <- tmp_BCR$primary
colnames(tmp_BCR_mx) <- gsub("proteomics_", "", colnames(tmp_BCR_mx))
...
pheatmap(tmp_BCR_mx[(tmp_BCR_anno %>% rownames_to_column() %>% arrange(PG, IGHV) %>% .$rowname), ], 
         ...
         gaps_row = (which(!tmp_BCR_anno %>% rownames_to_column() %>% arrange(PG, IGHV) %>% .$PG %>% duplicated())-1)[-1])
```
...
tmp_splice <- wideFormat(multiomics_MAE[splice_genes, ,"proteomics"], colDataCols = c("PG", "IGHV")) %>% as_tibble()
tmp_splice_mx <- tmp_splice %>% dplyr::select(-primary, -PG, -IGHV) %>% as.matrix()
rownames(tmp_splice_mx) <- tmp_splice$primary
colnames(tmp_splice_mx) <- gsub("proteomics_", "", colnames(tmp_splice_mx))
tmp_splice_anno <- tmp_splice %>% dplyr::select(PG, IGHV) %>% as.data.frame()
...
pheatmap(tmp_splice_mx[(tmp_splice_anno %>% rownames_to_column() %>% arrange( PG, IGHV) %>% .$rowname), ], 
         ...
         gaps_row = (which(!tmp_splice_anno %>% rownames_to_column() %>% arrange(PG, IGHV) %>% .$PG %>% duplicated())-1)[-1])
```
...
tmp_BCAA <- wideFormat(multiomics_MAE[some_BCAA, ,"proteomics"], colDataCols = c("PG", "IGHV" )) %>% 
  as_tibble()
tmp_BCAA_mx <- tmp_BCAA %>% dplyr::select(-primary, -PG, -IGHV) %>% as.matrix()
rownames(tmp_BCAA_mx) <- tmp_BCAA$primary
colnames(tmp_BCAA_mx) <- gsub("proteomics_", "", colnames(tmp_BCAA_mx))
tmp_BCAA_anno <- tmp_BCAA[, c("PG", "IGHV")] %>% as.data.frame()
rownames(tmp_BCAA_anno) <- tmp_BCAA$primary
tmp_BCAA_anno$PG <- factor(tmp_BCAA_anno$PG,levels = c(5,1:4,6))
tmp_BCAA_anno$IGHV[tmp_BCAA_anno$IGHV=="U"] <- "U-CLL"
tmp_BCAA_anno$IGHV[tmp_BCAA_anno$IGHV=="M"] <- "M-CLL"
...
tmp_proteasome <- wideFormat(multiomics_MAE[some_proteasome, ,"proteomics"], colDataCols = c("PG", "IGHV" )) %>% 
  as_tibble()
tmp_proteasome_mx <- tmp_proteasome %>% dplyr::select(-primary, -PG, -IGHV) %>% as.matrix()
rownames(tmp_proteasome_mx) <- tmp_proteasome$primary
colnames(tmp_proteasome_mx) <- gsub("proteomics_", "", colnames(tmp_proteasome_mx))
tmp_proteasome_anno <- tmp_proteasome[, c("PG", "IGHV")] %>% as.data.frame()
rownames(tmp_proteasome_anno) <- tmp_proteasome$primary
tmp_proteasome_anno$PG <- factor(tmp_proteasome_anno$PG,levels = c(5,1:4,6))
tmp_proteasome_anno$IGHV[tmp_proteasome_anno$IGHV=="U"] <- "U-CLL"
tmp_proteasome_anno$IGHV[tmp_proteasome_anno$IGHV=="M"] <- "M-CLL"
